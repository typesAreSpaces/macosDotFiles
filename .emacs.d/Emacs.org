...
Privacy concerns in macos requires to explicitly provide emacs, /usr/bin/ruby permission to the Full Disk in order to display files using Dired. Additionaly, the following utilities are required:
